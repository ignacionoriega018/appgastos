          ...
          {calendarDays.map((day, idx) => (
            <div 
              ...
              className={`
                ...
                ${!day ? 'bg-slate-50/30' : 'hover:bg-[var(--primary-50)]/30 cursor-pointer group'}
                ${day?.dateKey === new Date().toISOString().split('T')[0] ? 'bg-[var(--primary-50)]/50' : ''}
              `}
            >
              {day && (
                <>
                  <span className={`
                    ...
                    ${day.dateKey === new Date().toISOString().split('T')[0] ? 'bg-[var(--primary-600)] text-white' : 'text-slate-700'}
                  `}>
                    {day.day}
                  </span>
                  ...
                  <div className="flex flex-col gap-1 mt-2">
                    {day.hasExpenses && (
                      <div className="flex items-center gap-1 text-[10px] text-slate-600 font-medium bg-white border border-slate-100 rounded px-1.5 py-0.5 shadow-sm">
                        <div className="w-1.5 h-1.5 rounded-full bg-[var(--primary-500)]"></div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl flex flex-col max-h-[80vh] animate-slide-up">
            ...
            <div className="p-5 overflow-y-auto space-y-6">
              {/* Expenses Section */}
              <div>
                <h4 className="flex items-center gap-2 text-xs font-bold text-[var(--primary-600)] uppercase tracking-wider mb-3">
